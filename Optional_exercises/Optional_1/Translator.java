...
public class Translator {
    ...
    public void bool(int ltrue, int lfalse){
        
        int if_cond_true = code.newLabel();

        switch(look.tag){

            // PREDICT(N -> and Q Q) = FIRST(and) = { and }
            case Tag.AND:
                match(Tag.AND);
                boolp(if_cond_true,lfalse);
                code.emitLabel(if_cond_true);
                boolp(ltrue,lfalse);
                break;

            // PREDICT(N -> or Q Q) = FIRST(or) = { or }
            case Tag.OR:
                match(Tag.OR);
                boolp(ltrue,if_cond_true);
                code.emitLabel(if_cond_true);
                boolp(ltrue,lfalse);
                break;

            // PREDICT(N -> not Q) = FIRST(not) = { not }
            case '!':
                match(Token.not.tag);
                boolp(lfalse,ltrue);
                break; 
    ...
    // PREDICT(O -> relop H H) = FIRST(relop) = { relop }
    public void cond(int ltrue, int lfalse){
        switch(look.tag){
    ...
    public static void main(String[] args) {
        Lexer lex = new Lexer();
        String path = "Optional_exercises/Optional_1/Example.lft"; 
        try {
